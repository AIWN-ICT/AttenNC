"""Source-side neural network used by the DQN agent."""
...
class Network_S(nn.Module):
    """Attention-based Q-network for source coding decisions."""
    ...
    def __init__(self, state_dim, arg_dict, device):
        ...
        # Final merge MLP
        merge_in = num_test * 2  # self_out + aggregated path_out
    ...
    def forward(self, x):
        ...
        if (B == 1 and 
            ...
            stack = torch.stack(path_outs, dim=2)          # [B, num_test, parallel_path]
        ...
        # attention of self_out over path_outs
        ...
        # --- Merge ---
        merged = torch.cat([self_out, path_agg], dim=1)  # [B, num_test*2]
        h = F.relu(self.merge_fc1(merged))               # [B, num_units]
        h = F.relu(self.merge_fc2(h))                    # [B, num_units]
        out = self.merge_fc3(h)                          # [B, action_dim]
        return out
...
if __name__ == "__main__":
    ...
    # test with dynamic parallel_path
    for parallel_path in [1, 2, 3]:
        arg_dict['parallel_path'] = parallel_path
        print(f"\n=== Testing with parallel_path = {parallel_path} ===")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = Network_S(state_dim=None, arg_dict=arg_dict, device=device).to(device)
        test_input = torch.randint(0, 2, (1, R * parallel_path + 2, K)).float().to(device)
